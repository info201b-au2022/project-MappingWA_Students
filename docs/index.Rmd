...
most_insecure_counties <- paste(low_median_high_insecure, sep = ", ")

summary_table <- read.csv("https://raw.githubusercontent.com/info201b-au2022/project-MappingWA_Students/main/data/aggregate_data.csv")
```

## **ANALYSIS SUMMARY:** 

The top five counties that have the lowest median incomes and highest percentages of food insecure children in Washington are `r most_insecure_counties`. Whitman county faces the highest percentage of food insecure children as well as lowest median income households. A factor about student’s identities that affects their socioeconomic status is their race. The racial group with the highest percentage of people below the poverty level in Whitman county was `most_below_poverty_whitman` peoples. The racial group with the lowest percentage of people below the poverty level was `least_below_poverty_whitman`. Even further, racial identity connects with graduation rates across Washington. The county with the highest graduation rate in Washington is ‘county_highest_rate’. The lowest graduation rate in Washington is ‘lowest_grad_rate’.
